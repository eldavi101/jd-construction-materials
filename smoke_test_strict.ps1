...
try {
    ...
    $registerResponse = Invoke-WebRequest -Uri "http://localhost:3001/auth/register" -Method Post -Body $regBody -ContentType "application/json"
    Write-Host "Register Success: $($registerResponse.StatusCode)"
    
    Write-Host "Logging in..."
    $loginResponse = Invoke-RestMethod -Uri "http://localhost:3001/auth/login" -Method Post -Body $regBody -ContentType "application/json"
